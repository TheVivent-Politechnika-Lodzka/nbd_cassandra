...
# from flask_wtf import model_form
...
def createNode():
    try:
        ...
        os.system("sshpass -p '" + password + "' scp addnode.sh " +
                  username + "@" + ip + ":/home/" + username + "/")
        os.system("sshpass -p '" + password + "' ssh " + username + "@" +
                  ip + " 'echo " + password + " | bash addnode.sh " + username + "'")
        return jsonify(status="success")
    except:
...
def deleteNode(nodeid):
    ...
    if int(nodeid) <= c and int(nodeid) > h:
        os.system("sshpass -p '" + password + "' ssh " + username + "@" +
                  info[int(nodeid)] + " ./dsc-cassandra-2.1.11/bin/nodetool decommission")
        os.system(
            "/home/rohitsakala/dsc-cassandra-2.1.11/bin/nodetool removenode " + str(host[int(nodeid)]))
        return jsonify(status="Success")
    elif int(nodeid) <= h:
...
def getColumnFamilys(page, keyspaceid):
    ...
    if int(keyspaceid) < (len(rows) + 1) and int(keyspaceid) > 0:
        ...
        rows = session.execute(
            "SELECT * FROM schema_columnfamilies where keyspace_name='" + info['name'] + "'")
        info = []
        for x in range(len(rows)):
...
def deleteColumnFamily(columnfamilysid, keyspaceid):
    ...
    if int(keyspaceid) < (len(rows) + 1) and int(keyspaceid) > 0:
        ...
        rows = session.execute(
            "SELECT * FROM schema_columnfamilies where keyspace_name='" + info['name'] + "'")
        if int(columnfamilysid) < (len(rows) + 1) and int(columnfamilysid) > 0:
            query = "DROP TABLE " + info["name"] + \
                "." + rows[int(columnfamilysid) - 1][1]
            session.execute(query)
            return jsonify(status="success")
        else:
            return render_template('error.html', error="Not a valid columnfamilyid")
...
def createColumnFamily(keyspaceid):
    ...
    if field1 == None or field2 == None or field3 == None or name == None:
        ...
        if int(keyspaceid) < (len(rows) + 1) and int(keyspaceid) > 0:
            ...
            query = "CREATE TABLE " + name + "( " + field1 + " " + field1_type + " PRIMARY KEY," + \
                field2 + " " + field2_type + " ," + field3 + " " + field3_type + " )"
            session.execute(query)
...
def getEntrys(page, keyspaceid, columnfamilyid):
    ...
    if int(keyspaceid) < (len(rows) + 1) and int(keyspaceid) > 0:
        ...
        info = rows
        rows = session.execute("SELECT * FROM system.schema_columns WHERE keyspace_name = '" +
                               keyspacename + "' AND columnfamily_name = '" + columnfamilyname + "'")
        fields = []
        for i in rows:
            fields.append(i)
...
def createEntry(keyspaceid, columnfamilyid):
    ...
    if int(keyspaceid) < (len(rows) + 1) and int(keyspaceid) > 0:
        ...
        for key, value in dataDict.items():
            values.append(value)
        rows = session.execute("INSERT INTO " + columnfamilyname + " (" +
                               fields[0] + ", " + fields[1] + ", " + fields[2] + ") VALUES('" + values[0] + "', '" + values[1] + "', '" + values[2] + "')")
    else:
        return render_template('error.html', error="Not a valid keyspaceid")
    return "success"
...
def getEntrysInfo(keyspaceid, columnfamilyid, entryname):
    ...
    if int(keyspaceid) < (len(rows) + 1) and int(keyspaceid) > 0:
        ...
        query = "SELECT * FROM " + columnfamilyname + \
            " WHERE " + primarykey + "='" + entryname + "'"
        rows = session.execute(query)
        info = rows
        query = "SELECT * FROM system.schema_columns WHERE keyspace_name='" + \
            keyspacename + "' AND columnfamily_name = '" + columnfamilyname + "'"
        rows = session.execute(query)
        fields = []
        for i in rows:
...
def deleteEntry(columnfamilysid, keyspaceid, entryname):
    ...
    if int(keyspaceid) < (len(rows) + 1) and int(keyspaceid) > 0:
        ...
        primarykey = primarykey[17][2:]
        primarykey = primarykey[:-2]
        query = "DELETE FROM " + columnfamilyname + \
            " WHERE " + primarykey + "='" + entryname + "'"
        rows = session.execute(query)
        return "success"
